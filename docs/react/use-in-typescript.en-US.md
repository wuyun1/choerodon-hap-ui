...
Let's create a TypeScript project by using `create-react-app`, then import `choerodon-hap-ui` step by step.
...
Create a new project named `choerodon-hap-ui-demo-ts` using yarn.
...
Then we go inside `choerodon-hap-ui-demo-ts` and start it.
...
We are successfully running choerodon-hap-ui components now but in the real world, there are still lots of problems about choerodon-hap-ui-demo-ts.
