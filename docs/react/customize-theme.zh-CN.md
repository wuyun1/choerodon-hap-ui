...
choerodon-hap-ui 的样式使用了 [Less](http://lesscss.org/) 作为开发语言，并定义了一系列全局/组件的样式变量，你可以根据需求进行相应调整。
...
- 样式必须加载 less 格式。
  ...
  - 如果你是通过 `'choerodon-hap-ui/dist/choerodon-hap-ui.css'` 引入样式的，改为 `choerodon-hap-ui/dist/choerodon-hap-ui.less`。
...
建立一个单独的 `less` 文件如下，再引入这个文件。
   ...
   ```css
   @import "~choerodon-hap-ui/dist/choerodon-hap-ui.less";   // 引入官方提供的 less 样式入口文件
